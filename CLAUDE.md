...
**内容即数据，不是代码。** 频繁变更的站点文案集中在 `src/lib/config.js`：电话号码、抖音主页、`services`、`featuredCases`、`serviceAreas`、FAQ 主题简介。首页 `src/app/page.js` 直接从这些导出渲染。修改业务信息 = 改 `config.js`，通常不动 JSX。
